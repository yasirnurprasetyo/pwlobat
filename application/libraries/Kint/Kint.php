...
class Kint {
     ...
     * @return string
     */
    private static function _dump(&$var, $level = 0, $index = 0) {
        ...
        if ($var === NULL) {
            ...
            return '<var>bool</var>  <non-string>' . ( $var ? 'TRUE' : 'FALSE' ) . '</non-string>';
        } elseif (is_float($var)) {
            return '<var></var> <non-string>' . $var . '</non-string>';
        } elseif (is_int($var)) {
            return '<var></var> <non-string>' . $var . '</non-string>';
            ...
            if (strlen($var) > self::$maxStrLength) {
                ...
                return "<var></var><petik>\"</petik><dfn>{$html($var, 0)}</dfn><petik>\"</petik>";
//                return "<var>string</var>({$strlen($var)}) \"{$html($var, 0)}\"";
            ...
            if (empty($var)) {
                ...
                foreach ($var as $key => & $val) {
                    ...
                    $key = $isSeq ? "" : "<petik>\"</petik><dfn>{$html($key, 0)}</dfn><petik>\"</petik> <petik>=></petik>";


                    $dump = self::_dump($val, $level + 1, $idx++);
            ...
            self::_collapsed(TRUE);
            return "<var>array : {$count($var)}</var><dd><dl>{$implode($output, '')}</dl></dd>";
            ...
            if (empty($array)) {
                ...
                foreach ($reflection->getProperties(ReflectionProperty::IS_STATIC) as $property) {
                    ...
                    $dump = self::_dump($value, $level + 1, $idx++);
                    list( $class, $plus ) = self::_collapsed();
...
//                        $access = "<var>public</var>";
                    ...
                    $output[] = "<dt{$class}>{$plus}{$access} <petik>\"</petik><dfn>{$key}</dfn><petik>\"</petik> <petik>=></petik> {$dump}";
                }
            ...
            self::_collapsed(TRUE);
            return "<var> #" . $index . "</var> <petik>=></petik>  <var>{$getClass($var)} : {$count($array)}</var><dd><dl>{$implode($output, '')}</dl></dd>";
//            return "<var> #" . $index . "object {$getClass($var)}</var> <span>{{$count($array)}}</span><dd><dl>{$implode($output, '')}</dl></dd>";
    ...
    private static function _isSequential(array $array) {
        return self::$hideSequentialKeys ? array_keys($array) === range(0, count($array) - 1) : false;
    }
    ...
    private static function _wrapStart($callee) {
        if (self::$colorCodeLoops && isset($callee['file'])) {
            ...
            if (isset(self::$_usedColors[$uid])) {
                ...
                self::$_usedColors[$uid] = $color ? $color : '#fff';
            }
    ...
    private static function _wrapEnd($callee, $prevCaller) {
        ...
        $calleeInfo = isset($callee['file']) ? 'Called from ' . call_user_func(self::$pathDisplayCallback, $callee['file'], $callee['line']) . $callingFunction : '';


        return "<span class='info'>{$calleeInfo}</span></div>";
    }
    ...
    private static function _getPassedNames($callee, $defaultName = 'literal') {
        ...
        $codePattern = empty($callee['class']) ? $callee['function'] : $callee['class'] . $callee['type'] . $callee['function'];
